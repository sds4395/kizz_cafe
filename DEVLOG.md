...
- [ ] 폐업 2곳(`키즈마루`,`봉봉키즈`) 삭제 — RLS delete 정책 없어 SQL Editor에서 수동 실행 필요
- [ ] **추가 폼 개편(핵심)**: 카카오 검색으로 존재 확인 → 공공데이터(소상공인)로 저장하는 합법 플로우. data.go.kr 키 숨김용 Vercel 서버리스 프록시 필요. 카카오 장소ID(저장 허용) 저장 시 ‘카카오로 보기’를 검색 대신 장소상세 직링크로 업그레이드 가능.
